...
class BiDAFQuestionAnswering(nn.Module):
    def __init__(
        ...
        bert_layer_combination: str = "last",
    ):
        super().__init__()
        ...
        self.bert_layer_combination = bert_layer_combination
    ...
    def _combine_bert_hidden_states(self, bert_outputs) -> torch.Tensor:
        hidden_states = getattr(bert_outputs, "hidden_states", None)
        if not hidden_states:
            return bert_outputs.last_hidden_state

        if self.bert_layer_combination == "first":
            return hidden_states[1]
        if self.bert_layer_combination == "sum_last_four":
            return torch.stack(hidden_states[-4:], dim=0).sum(dim=0)
        return hidden_states[-1]

    def _run_bert_encoder(
        self,
    ...
    def _select_masked_embeddings(
        ...
        if torch.any(lengths == 0):
            raise ValueError("Each sequence in the batch must include at least one selected token.")

        max_length = int(lengths.max().item())
        selected = embeddings.new_zeros((batch_size, max_length, hidden_size))
        selected_mask = torch.zeros((batch_size, max_length), dtype=torch.bool, device=embeddings.device)

        for row in range(batch_size):
            row_embeddings = embeddings[row][token_mask[row]]
            row_length = row_embeddings.size(0)
            selected[row, :row_length] = row_embeddings
            selected_mask[row, :row_length] = True
    ...
    def embed_tokens(
        ...
        if self.embedding_mode == "static":
            ...
            bert_outputs = self._run_bert_encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
                token_type_ids=token_type_ids,
            )
            embeddings = self.bert_projection(self._combine_bert_hidden_states(bert_outputs))
    ...
    def forward(
        ...
        if (
            self.embedding_mode == "bert"
            and self.bert_architecture == "legacy_bidaf"
            and input_ids is not None
            and attention_mask is not None
            and context_token_mask is not None
            and question_token_mask is not None
        ):
            pair_embeddings = self.embed_tokens(
                input_ids,
                attention_mask,
                token_type_ids=token_type_ids,
            )
            context_embeddings, context_mask = self._select_masked_embeddings(pair_embeddings, context_token_mask)
            question_embeddings, question_mask = self._select_masked_embeddings(pair_embeddings, question_token_mask)
        else:
