...
class QAFeature:
    ...
    question_token_mask: list[bool] | None = None
...
def build_bert_features(
    ...
    context_max_length: int,
    question_max_length: int,
    architecture: str = "joint_qa_transformers",
) -> tuple[list[QAFeature], int]:
    features: list[QAFeature] = []
    skipped = 0

    if architecture == "legacy_bidaf":
        ...
        for example in examples:
            ...
            input_ids = list(encoding.get("input_ids", []))
            if not input_ids:
                skipped += 1
                continue
            ...
            question_token_mask: list[bool] = []
            question_token_count = 0
            for offset, sequence_id in zip(full_offsets, sequence_ids):
                is_question = sequence_id == 0
                is_context = sequence_id == 1
                question_token_mask.append(is_question)
...
def pad_qa_batch(batch: list[QAFeature], pad_id: int) -> dict[str, torch.Tensor | list[dict[str, object]]]:
    if batch and batch[0].input_ids is not None:
        ...
        question_token_mask = torch.zeros((batch_size, max_input_len), dtype=torch.bool)
        ...
        for row, feature in enumerate(batch):
            ...
            feature_question_token_mask = feature.question_token_mask or [False] * input_length
            ...
            question_token_mask[row, :input_length] = torch.tensor(feature_question_token_mask, dtype=torch.bool)
