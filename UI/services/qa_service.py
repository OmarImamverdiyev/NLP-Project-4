...
def _prepare_legacy_bert_inputs(
    context: str,
    question: str,
    tokenizer,
    *,
    ...
    sequence_ids = encoding.sequence_ids()
    context_token_mask = [sequence_id == 1 for sequence_id in sequence_ids]
    question_token_mask = [sequence_id == 0 for sequence_id in sequence_ids]
    question_token_count = int(sum(question_token_mask))
    context_token_count = int(sum(context_token_mask))
    if question_token_count == 0 or context_token_count == 0:
        raise ValueError("Both context and question must contain at least one token after tokenization.")
    ...
    token_type_tensor = torch.tensor([token_type_ids], dtype=torch.long, device=device)
    context_token_mask_tensor = torch.tensor([context_token_mask], dtype=torch.bool, device=device)
    question_token_mask_tensor = torch.tensor([question_token_mask], dtype=torch.bool, device=device)
    context_offsets = [
        tuple(int(value) for value in offset)
        for offset, is_context in zip(encoding["offset_mapping"], context_token_mask)
        if is_context
    ]
    ...
    return {
        ...
        "token_type_ids": token_type_tensor,
        "context_token_mask": context_token_mask_tensor,
        "question_token_mask": question_token_mask_tensor,
    ...
    *,
    ...
    if loaded.embedding_mode == "bert":
        if _uses_joint_bert_qa(loaded):
            ...
            batch = _prepare_legacy_bert_inputs(
                context,
                question,
                loaded.tokenizer,
                context_max_length=loaded.context_max_length,
    ...
    with torch.inference_mode():
        if loaded.embedding_mode == "bert" and _uses_joint_bert_qa(loaded):
            ...
            valid_length = None
        elif loaded.embedding_mode == "bert":
            ...
            candidate_mask = None
            valid_length = int(batch["context_tokens_used"])
        else:
            start_logits, end_logits = loaded.model(
