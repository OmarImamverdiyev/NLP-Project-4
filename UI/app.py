...
def render_qa_panel() -> None:
    ...
    if embedding_mode == "static":
        ...
        architecture_options = {
            "Notebook-style Joint QA": "joint_qa_transformers",
            "BiDAF + BERT Embeddings": "legacy_bidaf",
        }
        selected_architecture = st.selectbox(
            "BERT architecture",
            list(architecture_options.keys()),
            key="qa_bert_architecture_select",
        )
        bert_architecture = architecture_options[selected_architecture]
        if bert_architecture == "legacy_bidaf":
            layer_options = {
                "Last layer": "last",
                "First layer": "first",
                "Sum last four": "sum_last_four",
            }
            selected_layer = st.selectbox(
                "Legacy BERT layer mix",
                list(layer_options.keys()),
                key="qa_bert_layer_combination_select",
            )
            bert_layer_combination = layer_options[selected_layer]
        ...
        freeze_bert = st.checkbox(
            "Freeze BERT backbone",
            value=bert_architecture == "legacy_bidaf",
        )
    ...
    command = build_qa_command(
        ...
        bert_layer_combination=bert_layer_combination,
